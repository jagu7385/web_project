...
<div class="contact content">
    ...
    foreach($result2 as $row) {
        // Check if the current event type matches the booked service
        $isSelected = '';
        if(isset($_GET['bookid'])) {
            $selectedServiceId = $_GET['bookid'];
            if($selectedServiceId == $row->ServiceID) {
                $isSelected = 'selected';
            }
        }
        echo '<option value="' . htmlentities($row->EventType) . '" ' . $isSelected . '>' . htmlentities($row->EventType) . '</option>';
    }{
    ?>
